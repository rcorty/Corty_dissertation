...
\WV{One underlying assumption is that the vast majority of the genome, say greater than 99\%, does not have any appreciable effect on the phenotype.}{Nope, this is not a necessary assumption at all.}
...
There are fundamentally two types of genetic mapping: linkage mapping, in which pedigree information informs the statistical analysis, and association mapping, which is conducted in the absence of pedigree information.
Each can be applied in humans as well as in model organisms and commercially-important organisms.
There are principally \WV{two types of linkage analysis:}{You are stating this as if it were an contested fact when really you are choosing to distinguish types in this way. You could be less definitive by saying ``linkage analysis can be divided into...'' etc. However, see later comments first.}
\begin{enumerate}
	...
	\item \textbf{Model organism linkage mapping} can be thought of as an ``idealization'' of human linkage mapping.\WV{}{Use of haplotype information as done for F2s, etc, is linkage mapping only sort of -- its better termed ``linkage disequilibrium mapping'' or ``LD mapping''. Other examples of LD mapping include the type of haplotype association done for CC, DO, etc.}
...
\end{enumerate}
Similarly, \WV{there are three types of association mapping:}{See earlier comment regarding definitive categorizations.}
\begin{enumerate}
	\item \WV{\textbf{Human association mapping}}{This is really GWAS. You should use that term. Note also that accounting for relatedness in GWAS is not uncommon.} is based on a large population of not-closely-related individuals.
	...
	Inbred strain association mapping has been applied productively in model organisms of human physiology and disease \citep{TheComplexTraitConsortium2004,MacKay2012,King2012} as well as in commercially-important crops \citep{McMullen2009,Bandillo2013}. \WV{}{No. ISAM is GWAS (ie, SNP-based) using inbred strains. The CC, DSPR, and other MPP mapping panels typically (though not exclusively) use LD mapping. In this list, you are confusing distinctions between types of populations with types of methods}
\end{enumerate}
...
}{You could summarize stuff out of Ronnegard and Valdar 2011, plus other vQTL paper intros}
