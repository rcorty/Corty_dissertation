...
This statistical model can be applied in association mapping and LD mapping in situations where all individuals in the populaiton are not equally-related.
In such populations, population structure and cryptic relatedness invalidate QTL mapping approaches that use the standard linear model (SLM).

By estimating so called ``random effects'' with arbitrary covariance structure, the LMM can accommodate differential relatedness in the mapping population.
The additional complexity of the LMM relative to the SLM presents a computational challenge in applying it in large scale analyses, \eg on a dense set of genome wide markers.
...
\end{align}
For completeness, it should be shown that $\widehat{\bbeta_r} = \widehat{\bbeta}$ and $\widehat{\lambda_r} = \widehat{\lambda}$.
\begin{align}
  \widehat{\bbeta_r}    &= (\bX_r \T \bX_r) \inv \bX_r \T \by_r\\
...
\end{align}
It is then straightforward to compare $\bV\inv$ with $\bM\T \bM$ to verify their equality.
\begin{align}
...
With the ability to rapidly fit the LMM with heteroskedastic residuals, it is important to test the properties of this testing procedure as compared to the existing procedure.
It is to be expected that, when the residuals truly are homoskedastic and their variances are known by oracle, the heteroskedastic LMM should outperform the homoskedastic LMM in two ways.

First, I have observed that, when the homoskedastic LMM is applied to data with heteroskedastic residuals, the false positive rate (FPR) is coverly inflated.
Said another way, while the probability of observing a nominal $p$ value less than $c$ under the null should be $c$, in this scenario it is greater than $c$.
